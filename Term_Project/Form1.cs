...
namespace Term_Project
{
    ...
    public partial class Form1 : Form
    {
        ...
        private void Form_Load(object sendor, EventArgs e)
        {
            for (int i = 0; i < listBox2.Items.Count; i++)
            {
                int value = purposeList[i].count;
                this.values[i] = (float)value;
            }
        }
        private void Form_Paint(object sendor, PaintEventArgs e)
        {
            ...
            MakeColor();
            DrawPieChart(e.Graphics, rectangle, this.Palette, this.pen, this.values);
        }
        private void DrawPieChart(Graphics graphics, Rectangle rectangle, SolidBrush[] Palette, Pen[] pen, float[] values)
        {
            float totalValue = values.Sum();
            float startAngle = 0;
            for (int i = 0; i < values.Length; i++)
            {
                float sweepAngle = values[i] * 360f / totalValue;
